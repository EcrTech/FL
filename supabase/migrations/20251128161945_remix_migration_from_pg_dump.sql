...
--
-- Name: get_agent_call_performance_report(uuid, timestamp with time zone, timestamp with time zone); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.get_agent_call_performance_report(p_org_id uuid, p_start_date timestamp with time zone, p_end_date timestamp with time zone DEFAULT now()) RETURNS TABLE(agent_id uuid, agent_name text, total_calls bigint, inbound_calls bigint, outbound_calls bigint, avg_duration_seconds numeric, total_duration_seconds numeric, answered_calls bigint, missed_calls bigint, disposition_breakdown jsonb)
    LANGUAGE plpgsql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
  ...
  SELECT
    cl.agent_id,
    COALESCE(p.first_name || ' ' || COALESCE(p.last_name, ''), 'Unknown Agent') as agent_name,
    COUNT(*) as total_calls,
    COUNT(*) FILTER (WHERE cl.direction = 'inbound') as inbound_calls,
    COUNT(*) FILTER (WHERE cl.direction = 'outbound') as outbound_calls,
    ROUND(AVG(COALESCE(cl.call_duration, 0))::numeric, 2) as avg_duration_seconds,
    SUM(COALESCE(cl.call_duration, 0)) as total_duration_seconds,
    COUNT(*) FILTER (WHERE cl.status = 'completed') as answered_calls,
    COUNT(*) FILTER (WHERE cl.status IN ('no-answer', 'busy', 'failed')) as missed_calls,
    COALESCE(
      jsonb_object_agg(
        COALESCE(cd.name, 'No Disposition'),
        disposition_counts.count
      ) FILTER (WHERE cd.name IS NOT NULL OR disposition_counts.count > 0),
      '{}'::jsonb
    ) as disposition_breakdown
  FROM call_logs cl
  LEFT JOIN profiles p ON p.id = cl.agent_id
  LEFT JOIN LATERAL (
    SELECT cd.name, COUNT(*) as count
    FROM call_logs cl2
    LEFT JOIN call_dispositions cd ON cd.id = cl2.disposition_id
    WHERE cl2.agent_id = cl.agent_id
      AND cl2.org_id = p_org_id
      AND cl2.started_at >= p_start_date
      AND cl2.started_at <= p_end_date
    GROUP BY cd.name
  ) disposition_counts ON true
  LEFT JOIN call_dispositions cd ON cd.name = disposition_counts.name
  WHERE cl.org_id = p_org_id
    AND cl.started_at >= p_start_date
    AND cl.started_at <= p_end_date
...
--
-- Name: redefine_data_repository unique_personal_email_per_org; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.redefine_data_repository
    ADD CONSTRAINT unique_personal_email_per_org UNIQUE (org_id, personal_email);
...
--
-- Name: idx_call_logs_agent_started; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_call_logs_agent_started ON public.call_logs USING btree (org_id, agent_id, started_at DESC);
...
--
-- Name: idx_call_logs_disposition; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_call_logs_disposition ON public.call_logs USING btree (org_id, disposition_id) WHERE (disposition_id IS NOT NULL);
...
--
-- Name: idx_inventory_items_brand; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_inventory_items_brand ON public.inventory_items USING btree (brand);
...
--
-- Name: idx_redefine_repository_personal_email; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_redefine_repository_personal_email ON public.redefine_data_repository USING btree (org_id, personal_email);
...
--
-- Name: contacts outbound_webhook_contact_created; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER outbound_webhook_contact_created AFTER INSERT ON public.contacts FOR EACH ROW EXECUTE FUNCTION public.trigger_outbound_webhook('contact_created');
...
--
-- Name: contacts track_contact_pipeline_movement; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER track_contact_pipeline_movement AFTER UPDATE ON public.contacts FOR EACH ROW EXECUTE FUNCTION public.track_pipeline_movement();
...
--
-- Name: forms trigger_auto_webhook_token; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trigger_auto_webhook_token BEFORE INSERT OR UPDATE ON public.forms FOR EACH ROW EXECUTE FUNCTION public.auto_generate_webhook_token();
...
--
-- Name: contacts webhook_contacts_insert; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER webhook_contacts_insert AFTER INSERT ON public.contacts FOR EACH ROW EXECUTE FUNCTION public.trigger_outbound_webhook_generic();
...
--
-- Name: contacts Admins and managers can delete contacts; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Admins and managers can delete contacts" ON public.contacts FOR DELETE TO authenticated USING (((org_id = public.get_user_org_id(auth.uid())) AND (EXISTS ( SELECT 1
   FROM public.user_roles
  WHERE ((user_roles.user_id = auth.uid()) AND (user_roles.role = ANY (ARRAY['admin'::public.app_role, 'super_admin'::public.app_role, 'sales_manager'::public.app_role, 'support_manager'::public.app_role])))))));
