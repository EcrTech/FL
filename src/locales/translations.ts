export const translations = {
  en: {
    // Header
    applyForLoan: "Apply for a Loan",
    completeSteps: "Complete 4 simple steps to submit your application",
    
    // Location
    capturingLocation: "Capturing location...",
    locationCaptured: "Location captured",
    locationRequired: "Location required",
    retryLocationCapture: "Retry Location Capture",
    
    // Steps
    step: "Step",
    personalDetails: "Personal Details",
    panVerification: "PAN Verification",
    aadhaarVerification: "Aadhaar Verification",
    videoKYC: "Video KYC",
    
    // Basic Info Step
    personalInformation: "Personal Information",
    enterBasicDetails: "Enter your basic details to get started",
    loanAmountRequired: "Loan Amount Required",
    enterAmount: "Enter amount (₹25,000 - ₹2,00,000)",
    minimumAmount: "Minimum ₹25,000 • Maximum ₹2,00,000",
    invalidAmount: "Please enter an amount between ₹25,000 and ₹2,00,000",
    tenureDays: "Tenure (Number of Days)",
    enterTenure: "Enter tenure in days (30 - 180)",
    minMaxTenure: "Minimum 30 days • Maximum 180 days",
    invalidTenure: "Please enter a tenure between 30 and 180 days",
    fullNameAsPan: "Full Name (as per PAN)",
    enterFullName: "Enter your full name",
    mobileNumber: "Mobile Number",
    enter10Digit: "Enter 10-digit mobile",
    sendOtp: "Send OTP",
    verified: "Verified",
    enterOtp: "Enter 6-digit OTP",
    verify: "Verify",
    resend: "Resend",
    emailOptional: "Email Address (Optional)",
    enterEmail: "Enter your email",
    
    // Consents
    consentSection: "Required Consents",
    householdIncomeConsent: "I confirm that I am the highest earning member of my household with an annual household income of less than ₹3,00,000",
    termsConsent: "I accept the Terms & Conditions and Privacy Policy",
    aadhaarConsent: "I consent to Aadhaar verification for identity confirmation",
    
    // PAN Step
    verifyPan: "Verify your PAN",
    panDescription: "Your PAN will be verified in real-time with government records",
    panNumber: "PAN Number",
    enterPan: "Enter 10-character PAN",
    verifying: "Verifying...",
    panVerified: "PAN Verified",
    verificationFailed: "Verification Failed",
    retryVerification: "Retry Verification",
    verifiedDetails: "Verified Details",
    nameOnPan: "Name on PAN",
    panStatus: "PAN Status",
    back: "Back",
    continue: "Continue",
    verifyingPan: "Verifying PAN...",
    
    // Aadhaar Step
    verifyAadhaar: "Verify your Aadhaar",
    aadhaarDescription: "Enter your 12-digit Aadhaar number for verification",
    aadhaarNumber: "Aadhaar Number",
    enterAadhaar: "0000 0000 0000",
    aadhaarVerified: "Aadhaar Verified",
    nameOnAadhaar: "Name",
    addressOnAadhaar: "Address",
    dateOfBirth: "Date of Birth",
    
    // Video KYC Step
    recordVideoKyc: "Record Video KYC",
    videoDescription: "Record a short video to complete identity verification",
    
    // Buttons
    continueToNext: "Continue",
    submit: "Submit Application",
    
    // Footer
    dataSecure: "Your data is 256-bit encrypted and secure",
    
    // Success
    applicationSubmitted: "Application Submitted!",
    successMessage: "Your loan application has been successfully submitted. Our team will review and contact you shortly.",
    applicationNumber: "Application Number",
    
    // Errors
    invalidReferralLink: "Invalid Referral Link",
    loadingApplication: "Loading application...",
  },
  hi: {
    // Header
    applyForLoan: "ऋण के लिए आवेदन करें",
    completeSteps: "अपना आवेदन जमा करने के लिए 4 सरल चरण पूरे करें",
    
    // Location
    capturingLocation: "स्थान प्राप्त किया जा रहा है...",
    locationCaptured: "स्थान प्राप्त हुआ",
    locationRequired: "स्थान आवश्यक है",
    retryLocationCapture: "स्थान फिर से प्राप्त करें",
    
    // Steps
    step: "चरण",
    personalDetails: "व्यक्तिगत विवरण",
    panVerification: "पैन सत्यापन",
    aadhaarVerification: "आधार सत्यापन",
    videoKYC: "वीडियो केवाईसी",
    
    // Basic Info Step
    personalInformation: "व्यक्तिगत जानकारी",
    enterBasicDetails: "शुरू करने के लिए अपना बुनियादी विवरण दर्ज करें",
    loanAmountRequired: "आवश्यक ऋण राशि",
    enterAmount: "राशि दर्ज करें (₹25,000 - ₹2,00,000)",
    minimumAmount: "न्यूनतम ₹25,000 • अधिकतम ₹2,00,000",
    invalidAmount: "कृपया ₹25,000 और ₹2,00,000 के बीच राशि दर्ज करें",
    tenureDays: "अवधि (दिनों में)",
    enterTenure: "दिनों में अवधि दर्ज करें (30 - 180)",
    minMaxTenure: "न्यूनतम 30 दिन • अधिकतम 180 दिन",
    invalidTenure: "कृपया 30 और 180 दिनों के बीच अवधि दर्ज करें",
    fullNameAsPan: "पूरा नाम (पैन के अनुसार)",
    enterFullName: "अपना पूरा नाम दर्ज करें",
    mobileNumber: "मोबाइल नंबर",
    enter10Digit: "10 अंकों का मोबाइल दर्ज करें",
    sendOtp: "OTP भेजें",
    verified: "सत्यापित",
    enterOtp: "6 अंकों का OTP दर्ज करें",
    verify: "सत्यापित करें",
    resend: "पुनः भेजें",
    emailOptional: "ईमेल पता (वैकल्पिक)",
    enterEmail: "अपना ईमेल दर्ज करें",
    
    // Consents
    consentSection: "आवश्यक सहमतियां",
    householdIncomeConsent: "मैं पुष्टि करता/करती हूं कि मैं अपने घर का सबसे अधिक कमाने वाला सदस्य हूं जिसकी वार्षिक घरेलू आय ₹3,00,000 से कम है",
    termsConsent: "मैं नियम और शर्तें तथा गोपनीयता नीति स्वीकार करता/करती हूं",
    aadhaarConsent: "मैं पहचान की पुष्टि के लिए आधार सत्यापन की सहमति देता/देती हूं",
    
    // PAN Step
    verifyPan: "अपना पैन सत्यापित करें",
    panDescription: "आपके पैन को सरकारी रिकॉर्ड से रीयल-टाइम में सत्यापित किया जाएगा",
    panNumber: "पैन नंबर",
    enterPan: "10 अक्षरों का पैन दर्ज करें",
    verifying: "सत्यापित हो रहा है...",
    panVerified: "पैन सत्यापित",
    verificationFailed: "सत्यापन विफल",
    retryVerification: "पुनः सत्यापन करें",
    verifiedDetails: "सत्यापित विवरण",
    nameOnPan: "पैन पर नाम",
    panStatus: "पैन स्थिति",
    back: "वापस",
    continue: "जारी रखें",
    verifyingPan: "पैन सत्यापित हो रहा है...",
    
    // Aadhaar Step
    verifyAadhaar: "अपना आधार सत्यापित करें",
    aadhaarDescription: "सत्यापन के लिए अपना 12 अंकों का आधार नंबर दर्ज करें",
    aadhaarNumber: "आधार नंबर",
    enterAadhaar: "0000 0000 0000",
    aadhaarVerified: "आधार सत्यापित",
    nameOnAadhaar: "नाम",
    addressOnAadhaar: "पता",
    dateOfBirth: "जन्म तिथि",
    
    // Video KYC Step
    recordVideoKyc: "वीडियो केवाईसी रिकॉर्ड करें",
    videoDescription: "पहचान सत्यापन पूरा करने के लिए एक छोटा वीडियो रिकॉर्ड करें",
    
    // Buttons
    continueToNext: "जारी रखें",
    submit: "आवेदन जमा करें",
    
    // Footer
    dataSecure: "आपका डेटा 256-बिट एन्क्रिप्टेड और सुरक्षित है",
    
    // Success
    applicationSubmitted: "आवेदन जमा हो गया!",
    successMessage: "आपका ऋण आवेदन सफलतापूर्वक जमा हो गया है। हमारी टीम जल्द ही समीक्षा करके आपसे संपर्क करेगी।",
    applicationNumber: "आवेदन संख्या",
    
    // Errors
    invalidReferralLink: "अमान्य रेफरल लिंक",
    loadingApplication: "आवेदन लोड हो रहा है...",
  }
};

export type Language = 'en' | 'hi';
export type TranslationKey = keyof typeof translations.en;
